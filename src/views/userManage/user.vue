    ...
    <div style="margin-top:30px;">
      <div>
        ...
        <el-table :data='tableData' 
          ...
          <el-table-column label="操作" align='center' width="220vw">
            <template slot-scope="scope">
              <!-- @click.native:监听根元素的原生事件 -->
              <el-button size="mini" type="text" plain @click.native="Resetpow(scope.row)">重置密码</el-button>
  ...
  methods: {
    ...
    Resetpow(row){
      ...
      this.$prompt("请输入重置密码","提示",{
        ...
        cancelButtonText: '取消',
        // /^[0-9A-Za-z]{6,}$/ 不少于6位数
        inputPattern:/^(?![A-Za-z]+$)(?![A-Z\d]+$)(?![A-Z\W]+$)(?![a-z\d]+$)(?![a-z\W]+$)(?![\d\W]+$)\S{8,20}$/,
        inputErrorMessage: '密码长度至少8位,包含大小写字母、数字、特殊字符,任意三种'
      }).then(({value}) => {
        // api.Password(row.id,value).then(res => {
          console.log("重置密码为" + value)
        // })
      }).catch(() => {
        this.$message({
          type: 'info',
